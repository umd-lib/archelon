...
class PublishJobStatusUpdatedJob < ApplicationJob
  ...
  def perform(publish_job)
    PublishJobChannel.broadcast_to(publish_job, job: publish_job, statusWidget: status_widget(publish_job))
  end

  private

    def status_widget(publish_job)
      ActionController::Renderer.for(PublishJobController).render(
