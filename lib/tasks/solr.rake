...
namespace :solr do # rubocop:disable Metrics/BlockLength
  ...
  task start_server: :environment do
    ...
    unless e.message.include?("Port #{@solr_instance.port} is already being used by another process")
    ...
    puts "  b) NO: Either set SOLR_OPTIONS[:port] to a different value or stop the process that's using port " \
         "#{@solr_instance.port}."
  end
