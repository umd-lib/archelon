...
4.1) Log in at <http://fcrepo-local:8080/user/>

4.2) Go to <http://fcrepo-local:8080/rest/>
...
6.3) Edit the "docker-plastron.yml" file:
...
6.4) Deploy the "plastrond" Docker stack:
...
8.2) Copy the "docker-archelon-localhost-template.env" to "docker-archelon.env"

```bash
> cp docker-archelon-localhost-template.env docker-archelon.env
```
